...
# 4. TRAIN/TEST SPLIT (without stratification due to single samples)
split = train_test_split(
    X, y_disease, y_cause, y_prevention,
    test_size=TEST_SIZE, 
    random_state=42
    # Removed stratify because many classes have only 1 sample
)
...
with torch.no_grad():
    for i in range(min(10, len(X_test))):
        ...
        actual = disease_encoder.inverse_transform([y_disease_test[i]])[0]
        predicted = disease_encoder.inverse_transform([pred.cpu().numpy()[0]])[0]
        ...
        print(f"   Predicted: {predicted} ({conf.item()*100:.1f}%) {'✅' if is_correct else '❌'}")
